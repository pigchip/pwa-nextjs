...
const Tutorial = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-between min-h-screen mx-auto p-6 bg-white">
      ...
      <div className="flex justify-center w-full mt-16 mb-8"> {/* Espaciado adicional con mt-16 */}
        <div className="w-64 h-64 bg-gray-200 flex items-center justify-center rounded-md overflow-hidden">
          <img
            src={steps[currentStep].gif}
            alt="Tutorial Gif"
            className="object-contain w-full h-full bg-white" // Mantener el GIF dentro del contenedor con el mismo tamaño
          />
        </div>
      </div>
      ...
      <div className="text-center">
        <h2 className="text-2xl font-bold text-black mb-4">{steps[currentStep].title}</h2>
        <p className="text-gray-600 mb-8 text-[#6C7976]">{steps[currentStep].description}</p>
      </div>
      ...
      <div className="w-full">
        ...
        <button className="skip-btn text-gray-500 w-full text-center">Saltar</button>
      </div>
    </div>
  );
};
