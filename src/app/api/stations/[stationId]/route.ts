// pages/api/stations/[stationId]/route.ts
...
async function getStationById(stationId: string): Promise<any> {
  ...
  if (!response.ok) {
    ...
    throw new FetchError(response.status, errorData.message || 'Failed to fetch station');
  }
