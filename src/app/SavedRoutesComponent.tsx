...
const SavedRoutesComponent: React.FC = () => {
  ...
  const { setSelectedItinerary, setActiveComponent } = useContext(SelectedItineraryContext);
        ...
        {savedRoutes.length === 0 ? (
          ...
          <ul className="space-y-6 md:space-y-8">
            {savedRoutes.map((itinerary, index) => (
              <li key={index} className="bg-green-100 p-4 md:p-6 rounded-lg shadow-lg border border-gray-200">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
                  <div className="w-full md:w-2/3 space-y-2 mb-4 md:mb-0">
                    <p className="text-lg font-semibold text-gray-700">Ruta {index + 1}</p>
                    <p className="text-gray-600">Inicio: <span className="font-medium">{itinerary.startNameIti}</span></p>
                    <p className="text-gray-600">Destino: <span className="font-medium">{itinerary.endNameIti}</span></p>
                    <p className="text-gray-600">Duración: <span className="font-medium">{formatDuration(itinerary.duration)}</span></p>
                    <p className="text-gray-600">Distancia a pie: <span className="font-medium">{Math.round(itinerary.walkDistance)}m</span></p>
                  </div>
                ...
                <div className="mt-4 flex flex-wrap items-center space-x-4 md:space-x-6">
                  {itinerary.legs.map((leg, legIndex) => (
                    <div key={legIndex} className="flex items-center mb-3">
                      <div
                        style={{
                          backgroundColor: leg.route?.color ? `#${leg.route.color}` : leg.mode === 'WALK' ? '#00BFFF' : 'gray',
                        }}
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-700">{leg.mode}</p>
